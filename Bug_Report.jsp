...
//ResultSet rs;
int i = st.executeUpdate("insert into bug_report(bug_name,bug_type, bug_level, priority, pname, testercode, bugdate, e_code, status, bug_rectifieddate,status1) values ('"+Bug_Name+"','" + Bug_Type + "','" + Bug_Level + "','"+Priority+"','"+Pname+"','"+ TesterCode+"','" + Bug_Date + "', '"+Employeecode+"','"+Status+"','"+BugRectifieddate+"','" + Status1+ "')");

if(i>=1)
{
	response.sendRedirect("Bug Report.jsp");	
}
else{
	out.print("error");
}
